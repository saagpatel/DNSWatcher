...
func TestResolveClientIdentity(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "malformed xff hop fails closed to the trusted peer",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
		...
		{
			name:       "malformed xff with garbage hop fails closed to the peer",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
		...
		{
			name:       "forwarded element without for fails closed to the peer",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
		...
		{
			name:       "empty xff hop fails closed to the peer",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"X-Forwarded-For": []string{"198.51.100.7,,10.0.0.8"},
			},
		...
		{
			name:       "malformed forwarded poisons otherwise valid xff",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"Forwarded":       []string{"for=198.51.100.7, by=10.0.0.8"},
				"X-Forwarded-For": []string{"198.51.100.7, 10.0.0.8"},
		...
		{
			name:       "duplicate forwarded for param fails closed",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"Forwarded": []string{"for=198.51.100.7;for=203.0.113.1"},
			},
			trusted:         trusted,
			wantAddress:     "10.0.0.2",
			wantSource:      clientSourceRemoteAddr,
		...
		{
			name:       "invalid forwarded param without equals fails closed",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"Forwarded": []string{"for=198.51.100.7;notaparam"},
			},
			trusted:         trusted,
			wantAddress:     "10.0.0.2",
			wantSource:      clientSourceRemoteAddr,
		...
		{
			name:       "empty forwarded for param fails closed",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"Forwarded": []string{"for="},
			},
			trusted:         trusted,
			wantAddress:     "10.0.0.2",
			wantSource:      clientSourceRemoteAddr,
		...
		{
			name:       "duplicate forwarded by param fails closed",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"Forwarded": []string{"for=198.51.100.7;by=10.0.0.8;by=10.0.0.9"},
			},
			trusted:         trusted,
			wantAddress:     "10.0.0.2",
			wantSource:      clientSourceRemoteAddr,
		...
		{
			name:       "same-element forwarded by param remains valid",
			remoteAddr: "10.0.0.2:80",
			header: http.Header{
				"Forwarded": []string{"for=198.51.100.7;by=10.0.0.8"},
			},
